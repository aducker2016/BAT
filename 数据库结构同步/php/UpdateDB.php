...
function compare_column($source_link, $target_link, $source_db, $target_db, $table_name){
	...
	foreach($source_column as $column_name => $column_info){
		if(!$target_column[$column_name]){
			//插入字段
			$sql = 'alter table '.$target_db.'.'.$table_name.' add '.$column_info;
			execute($sql, $target_link);
		} else {
			//修改字段
			if($column_info != $target_column[$column_name]){
				$sql = 'alter table '.$target_db.'.'.$table_name.' modify '.$column_info;
				execute($sql, $target_link);
			}
		}
	}
...
function get_table_column($link, $db, $table){
	...
	$rs = mysql_query($sql, $link);
	while ($row = mysql_fetch_assoc($rs)) {
		$create_table = trim($row['Create Table']);
	}

	$table_column = array();
	$temp_columns = explode("\n", $create_table);
	foreach($temp_columns as $temp_column){
		if( substr($temp_column,0,3) == "  `" ){
			$temp_key = explode("`", $temp_column);
			$temp_val = explode(",", $temp_column);
			$table_column[$temp_key[1]] = $temp_val[0].($before_column == '' ? '' : ' after '.$before_column);
			$before_column = $temp_key[1];
		}
	}
